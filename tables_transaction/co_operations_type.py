...
st = s.Table("co_operations_type", metadata, autoload=True, autoload_with=engine) #co_operations_type
...
def select_table():
    # the query object
    query = s.select([st.columns.co_operations_type_id, st.columns.co_operations_type_desc])
    # execute query
    select_result_proxy = connection.execute(query)
    return select_result_proxy

def update_table():
    co_operations_type_id = int(input("Select co_operations_type id: "))
    co_operations_type_desc = str.upper(input("Insert co_operations_type desc: "))

    update = s.update(st).values(co_operations_type_desc=co_operations_type_desc).where(st.columns.co_operations_type_id  == co_operations_type_id)
    proxy = connection.execute(update)
    ans = "selected id updated"
    return ans

def delete_table():
    co_operations_type_id = int(input("Select co_operations_type_id: "))
    query = s.delete(st).where(st.columns.co_operations_type_id == co_operations_type_id)
    proxy = connection.execute(query)
    ans = "selected id deleted"
    return ans

def insert_table():
    co_operations_type_desc = str.upper(input("Insert co_operations_type_desc: "))
    insert = s.insert(st).values(co_operations_type_desc=co_operations_type_desc)
    proxy = connection.execute(insert)
    ans = f"{co_operations_type_desc} inserted"
...
if __name__ == "__main__":
    display_table("co_operations_type")
