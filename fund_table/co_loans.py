...
st = s.Table("co_loans", metadata, autoload=True, autoload_with=engine) #project_category
...
def select_table():
    # the query object
    query = s.select([st.columns.co_loans_id, st.columns.co_loans_desc])
    # execute query
    select_result_proxy = connection.execute(query)
    return select_result_proxy

def update_table():
    co_loans_id = int(input("Select co_loans id: "))
    co_loans_desc = str.upper(input("Insert co_loans desc: "))

    update = s.update(st).values(co_loans_desc=co_loans_desc).where(st.columns.co_loans_id  == co_loans_id)
    proxy = connection.execute(update)
    ans = "selected id updated"
    return ans

def delete_table():
    co_loans_id = int(input("Select co_loans_id: "))
    query = s.delete(st).where(st.columns.co_loans_id == co_loans_id)
    proxy = connection.execute(query)
    ans = "selected id deleted"
    return ans

def insert_table(co_company_id,bee_no,co_loan_type_id):
    insert = s.insert(st).values(co_company_id=co_company_id,bee_no=bee_no,co_loans_type_id=co_loan_type_id)
...
def display_table(table_name,co_company_id,bee_no,co_loan_type_id) -> tuple:
...
def retrieve_co_loans_id():
    # retrieve the id of the project just inserted
     # the query object
    query = s.select([st.columns.co_loans_id]).order_by(s.desc(st.columns.co_loans_id)).limit(1)
    # execute query
    select_result_proxy = connection.execute(query)

    for result in select_result_proxy:
        co_loans_id = result[0]
        
    return co_loans_id


if __name__ == "__main__":
    display_table("co_loans",2)
