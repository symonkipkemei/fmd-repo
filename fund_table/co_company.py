...
st = s.Table("co_company", metadata, autoload=True, autoload_with=engine) #project_category
...
def select_table():
    # the query object
    query = s.select([st.columns.co_company_id, st.columns.co_company_desc])
    # execute query
    select_result_proxy = connection.execute(query)
    return select_result_proxy

def update_table():
    co_company_id = int(input("Select co_company id: "))
    co_company_desc = str.upper(input("Insert co_company desc: "))

    update = s.update(st).values(co_company_desc=co_company_desc).where(st.columns.co_company_id  == co_company_id)
    proxy = connection.execute(update)
    ans = "selected id updated"
    return ans

def delete_table():
    co_company_id = int(input("Select co_company_id: "))
    query = s.delete(st).where(st.columns.co_company_id == co_company_id)
    proxy = connection.execute(query)
    ans = "selected id deleted"
    return ans

def insert_table(co_fund_id):
    co_company_desc = str.upper(input("Insert co_company_desc: "))
    insert = s.insert(st).values(co_company_desc=co_company_desc,co_fund_id=co_fund_id)
    proxy = connection.execute(insert)
    ans = f"{co_company_desc} inserted"
