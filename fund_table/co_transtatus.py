...
st = s.Table("co_transtatus", metadata, autoload=True, autoload_with=engine) #project_category
...
def select_table():
    # the query object
    query = s.select([st.columns.co_transtatus_id, st.columns.co_transtatus_desc])
    # execute query
    select_result_proxy = connection.execute(query)
    return select_result_proxy

def update_table():
    co_transtatus_id = int(input("Select co_transtatus id: "))
    co_transtatus_desc = str.upper(input("Insert co_transtatus desc: "))

    update = s.update(st).values(co_transtatus_desc=co_transtatus_desc).where(st.columns.co_transtatus_id  == co_transtatus_id)
    proxy = connection.execute(update)
    ans = "selected id updated"
    return ans

def delete_table():
    co_transtatus_id = int(input("Select co_transtatus_id: "))
    query = s.delete(st).where(st.columns.co_transtatus_id == co_transtatus_id)
    proxy = connection.execute(query)
    ans = "selected id deleted"
    return ans

def insert_table():
    co_transtatus_desc = str.upper(input("Insert co_transtatus_desc: "))
    insert = s.insert(st).values(co_transtatus_desc=co_transtatus_desc)
    proxy = connection.execute(insert)
    ans = f"{co_transtatus_desc} inserted"
